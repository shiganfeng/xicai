  ...
  <div class="policy-info">
    <el-row class="myxue">
      <el-breadcrumb>
        ...
        <el-breadcrumb-item :to="{ path: '/chuangInfo' }"><span class="sgftitle">{{$route.query.id ? '编辑' : '新增'}}</span></el-breadcrumb-item>
      </el-breadcrumb>
    </el-row>
    ...
    :model="ruleForm" :rules="rules" label-width="210px" v-loading="loading">
      ...
      <el-row>
        ...
        <el-form-item class="myItem">
          <el-button @click="$router.push('/ChuangrongList')" class="mycancle" type="danger">取消</el-button>
          ...
          let adddata = await CRAddInfo(requestData).then((response) => {
            ...
            setTimeout(() => {
              ...
              setTimeout( () => {
                loading.close();
                //页面跳转到list页面
                this.$router.push('/ChuangrongList')
              },2000)
            }, 2000);     
            console.log(data)
          }).catch((error) => {

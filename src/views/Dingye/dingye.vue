            ...
            <template>
              <template>
                <el-tooltip effect="dark" content="编辑" placement="top" :enterable="false" >
                  <router-link :to="`/dingyeInfo?id=${row.xid}`">
...
<script>
import { CRShaixuan,CRGetEnterList,CRDeleteInfo,CRRetest,CRGetEditInfo,CRGetDingyeList,CRDingyeShaixuan } from '../../api/login.js'
export default {
  data() {
    return {
      total: 0,
  ...
  methods: {
    ...
    async getEnterList(){
          ...
          console.log(this.sgfPage)
          let mydata = await CRGetDingyeList(requestData)
          console.log(mydata)
    ...
    async shaixuan(){
      ...
      console.log(requestData)
          let shaixuandata = await CRDingyeShaixuan(requestData).catch((res) => {
            console.log(res)
          })

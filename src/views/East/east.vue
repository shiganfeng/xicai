            ...
            <template>
              <template>
                <el-tooltip effect="dark" content="编辑" placement="top" :enterable="false" >
                  <router-link :to="`/eastInfo?id=${row.xid}`">
...
<script>
import { CRShaixuan,CRGetEnterList,CRDeleteInfo,CRRetest,CRGetEditInfo,CRGetDongzhanList,CRDongzhanShaixuan } from '../../api/login.js'
export default {
  data() {
    return {
      total: 0,
  ...
  methods: {
    ...
    async getEnterList(){
          ...
          console.log(this.sgfPage)
          let mydata = await CRGetDongzhanList(requestData)
          console.log(mydata)
    ...
    async shaixuan(){
      ...
      console.log(requestData)
          let shaixuandata = await CRDongzhanShaixuan(requestData).catch((res) => {
            console.log(res)
          })

            ...
            <template>
              <template>
                <el-tooltip effect="dark" content="编辑" placement="top" :enterable="false" >
                  <router-link :to="`/rencaiInfo?id=${row.xid}`">
...
<script>
import { GYShaixuan,GYGetEnterList,GYDeleteInfo,GYRetest,GYGetEditInfo } from '../../api/login.js'
export default {
  data() {
    return {
      total: 0,
  ...
  methods: {
    ...
    async tryAgain(row){
      ...
      let retestData = await GYRetest(row.xid).then( (res) => {
        console.log(res.data)
      }).catch( (rej) => {
        console.log(rej)
      })
      
      let bigdata = await GYGetEditInfo(row.xid)
      ...
      //调用删除接口
        ...
        if(this.deleteId.length == 0){
          ...
          if (confirm) {
            ...
            let deletedate = await GYDeleteInfo(requestData)
            if (deletedate) {
    ...
    async getEnterList(){
          ...
          console.log(this.sgfPage)
          let mydata = await GYGetEnterList(requestData)
          console.log(mydata)
    ...
    async shaixuan(){
      ...
      console.log(requestData)
          let shaixuandata = await GYShaixuan(requestData).catch((res) => {
            console.log(res)
          })

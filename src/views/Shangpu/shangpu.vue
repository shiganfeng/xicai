            ...
            <template>
              <template>
                <el-tooltip effect="dark" content="编辑" placement="top" :enterable="false" >
                  <router-link :to="`/shangpuInfo?id=${row.xid}`">
...
<script>
import { SPShaixuan,SPGetEnterList,SPDeleteInfo,SPRetest,SPGetEditInfo } from '../../api/login.js'
export default {
  data() {
    return {
      total: 0,
  ...
  methods: {
    ...
    async tryAgain(row){
      ...
      let retestData = await SPRetest(row.xid).then( (res) => {
        console.log(res.data)
      }).catch( (rej) => {
        console.log(rej)
      })
      
      let bigdata = await SPGetEditInfo(row.xid)
      ...
      //调用删除接口
        ...
        if(this.deleteId.length == 0){
          ...
          if (confirm) {
            ...
            let deletedate = await SPDeleteInfo(requestData)
            if (deletedate) {
    ...
    async getEnterList(){
          ...
          console.log(this.sgfPage)
          let mydata = await SPGetEnterList(requestData)
          console.log(mydata)
    ...
    async shaixuan(){
      ...
      console.log(requestData)
          let shaixuandata = await SPShaixuan(requestData).catch((res) => {
            console.log(res)
          })
